...
namespace Scarecrow
{
    ...
    public class Toggle_SwitchDrawer : MaterialPropertyDrawer
    {
        ...
        // 输入应该是"true" 或者 "false"
        public Toggle_SwitchDrawer(string _showDefaultValue)
        {
            showDefaultValue = (_showDefaultValue == "true");
        }
        ...
        public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
        {
            ...
            if (showDefaultValue)
            {
                _enbale = EditorGUI.Toggle(position, " ", (int) prop.floatValue == 0 ? false : true);
                // 调用默认值Drawer的label绘制。不支持Tex和Color。
                DefDrawer.DrawLabel(position, prop, new GUIContent(label), editor);
            }
            else
            {
